...
public class OtpController extends BaseController implements OtpControllerInterface {
	...
	public Otp nuovoOtp(Utente utente) {

		EntityManager entityManager = getEntityManager();
		EntityTransaction transaction = entityManager.getTransaction();

		try {
			Otp otp = creaOtp(utente);
			logger.info("sei nel Otp Controller insert >>>" + otp);
			transaction.begin();
			if (!entityManager.contains(otp)) {
				otp = entityManager.merge(otp);
	...
	public Otp creaOtp(Utente utente) throws InvalidKeyException, NoSuchAlgorithmException {
		Otp otp = new Otp();
		otp.setCodiceOtp(new GeneraOtp().generator());
		otp.setDataCreazione(LocalDateTime.now());
		otp.setDataScadenza(otp.getDataCreazione().plusDays(1));
		otp.setStato("attivo");
		otp.setUtente(utente);
		return otp;
